...
for (n in sample_sizes) {
  ...
  for (i in 1:N) {
    ...
    repeat {
      ...
      # Contamination
      contamination_indices <- order(mu, decreasing = TRUE)[1:n_cont]
      mu_cont <- 0.002
      y[contamination_indices] <- rbeta(n_cont, shape1 = mu_cont * phi[contamination_indices], shape2 = (1 - mu_cont) * phi[contamination_indices])      
    ...
    tests <- list(
      ...
      robust_gradient_statistic(fit_lsmle_h1$score, fit_lsmle$theta_hat, beta[2], fit_lsmle_h1$Vq$H, fit_lsmle_h1$Vq$J, c(2)),
      robust_gradient_statistic(fit_lmdpde_h1$score, fit_lmdpde$theta_hat, beta[2], fit_lmdpde_h1$Vq$H, fit_lmdpde_h1$Vq$J, c(2)),
  ...
  # Save results as txt
  file_name <- paste0("G:/Meu Drive/03 - Sim/03_case_study_1/Results/ScenarioB_Cont_n", n, ".txt")
...
# 
# pdf("../ScenarioB_Cont_n40_model.pdf", width = 8, height = 6)
# plot(x1, y, xlab = "X", ylab = "y", ylim = c(0, 0.3))
# # Pontos contaminados
...
# dev.off()
# 
# pdf("../ScenarioB_Cont_n40.pdf", width = 8, height = 6)
# plot(x1, y, xlab = "X", ylab = "y", ylim = c(0, 0.3))
# points(x1[contamination_indices], y[contamination_indices], col = "black", pch = 16)
# dev.off()
